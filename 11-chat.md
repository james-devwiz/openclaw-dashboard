# Phase 11: Chat — SSE Streaming with Topic Routing
...
### 11.9 Streaming Pipeline Reliability

Both `createGatewayStream()` in `lib/chat-stream.ts` and `buildSSEResponse()` in `lib/provider-stream.ts` always call `onComplete()` even when the stream accumulates no content — the `if (fullResponse)` guard that previously wrapped post-processing was removed because it silently dropped empty responses (~1 in 10 messages). SSE parse errors are now logged via `console.warn` instead of silently swallowed. If the response is empty, `ChatMessageList` shows an italic "No response received" fallback.

### 11.10 Delegated Task Tracking

When a user delegates substantial work in chat (research, analysis, reports), the system auto-creates a tracked task.

**Detection — `lib/chat-task-detect.ts`:**
- `shouldDetect(topic)` limits to `general`, `research`, `tasks` topics
- `detectDelegatedTask(message, topic)` sends user message to gateway (`gpt-5.1-codex-mini`, 5s timeout, max 80 tokens) to classify if work is being delegated
- Messages under 20 characters are skipped
- Returns `{ id, name }` if delegated, `null` otherwise

**Task lifecycle:**
1. On detection: creates task with status "In Progress", assignee "Jimmy AI", source "Chat"
2. On successful response: `completeDelegatedTask()` moves to "Needs Review"
3. On empty response: `failDelegatedTask()` logs note, task stays "In Progress"

**Wiring — `app/api/chat/route.ts`:**
- Detection runs after saving user message, before streaming (5s timeout)
- `delegated_task` metadata merged into `initialMeta` SSE event
- `onComplete` checks `fullResponse` to decide complete vs fail

**UI — `ChatMessageList.tsx`:**
- Blue "Task tracked: {name}" chip with `ListTodo` icon, links to `/goals`

### 11.11 File Attachments
...
- [ ] Streaming completes without "No response" when gateway returns a valid response
- [ ] Empty AI response shows italic "No response received" fallback (not blank bubble)
- [ ] Delegating complex work in general/research/tasks topics auto-creates an "In Progress" task
- [ ] "Task tracked" blue chip appears below the assistant message with correct task name
- [ ] Task moves to "Needs Review" when the AI response completes successfully
- [ ] Simple questions (greetings, quick lookups) do not trigger task creation
