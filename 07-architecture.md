# Phase 7: Architecture — Project Setup, Design System & Layout
...
- Nav items: Overview, Chat, Projects, Goals & Tasks, Leads, LinkedIn, Content Studio, Approvals (with pending badge), Activity, Memory, Documents, Briefs & Reports, Architecture, Heartbeat
...
- **Notification badges** — 4 nav items show red count badges (all poll every 30s):
  - **Approvals**: pending count via `useApprovals` hook
  - **Chat**: unread assistant messages via `useChatUnread` hook (see Phase 11)
  - **Briefs & Reports**: unread brief count via `useBriefUnread` hook — reuses `chat_read_cursors` table with topic `"_briefs"`. API: `GET/POST /api/briefs/unread`. Visiting the page calls `markRead()` to clear
  - **Goals & Tasks**: "Needs Review" task count via `useTaskReviewCount` hook. API: `GET /api/tasks/review-count`
- `BadgeKey` type: `"approvals" | "chat" | "briefs" | "tasks"`
...
- [ ] Briefs & Reports nav item shows red badge when new briefs exist since last visit
- [ ] Visiting the Briefs page clears the briefs badge
- [ ] Goals & Tasks nav item shows red badge when tasks are in "Needs Review" status
