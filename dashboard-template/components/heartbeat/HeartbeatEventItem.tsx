"use client" // Requires useState for expand/collapse and copy button state
...
export function parseHeartbeatSummary(summary: string): string[] {
  ...
  for (const ch of cleaned) {
    ...
    if (depth === 0 && ch === ";") { if (current.trim()) items.push(current.trim()); current = "" }
    else current += ch
  }
...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <Button variant="ghost" size="icon" onClick={handleCopy} className="size-7" aria-label="Copy heartbeat details">
...
function HeartbeatDetails({ items, status }: { items: string[]; status: string }) {
  return (
    <ul className="space-y-1" role="list">
      {items.map((item, i) => {
        const isIssue = status === "failure" || /fail|error|missing|anomal|unsupported|broken/i.test(item)
        const Icon = isIssue ? AlertTriangle : CheckCircle2
        const color = isIssue ? "text-amber-500" : "text-emerald-500"
        return (
          <li key={i} className="flex items-start gap-2 text-xs">
            <Icon size={12} className={cn(color, "shrink-0 mt-0.5")} aria-hidden="true" />
  ...
  return (
    <div className="border-t border-border pt-2 mt-2">
      ...
      <div className="space-y-1">
        {briefs.map((b) => (
          <Link key={b.id} href="/brief" className="flex items-center gap-2 text-xs text-blue-600 dark:text-blue-400 hover:underline">
            <FileText size={12} aria-hidden="true" />
            <span>{b.title}</span>
            <span className="text-muted-foreground">({b.briefType})</span>
          </Link>
        ))}
      </div>
    </div>
...
export default function HeartbeatEventItem({ event, linkedBriefs }: { event: HeartbeatEvent; linkedBriefs: LinkedBrief[] }) {
